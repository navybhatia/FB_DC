...
void Settings::saveSettings_client()
{
	...
	{ // Appearance
		saveOptionsView_gui(appearanceView);
		...
		{ // Colors and sounds
			...
			*/
			// Tab bolding
			saveOptionsView_gui(colorView);
		}

		{ // Window
			// Auto-open on startup
			saveOptionsView_gui(windowView1);

			// Window options
			saveOptionsView_gui(windowView2);

			// Confirm dialog options
			saveOptionsView_gui(windowView3);
		}
	}
	...
	{ // Advanced
		saveOptionsView_gui(advancedView);
 ...
 */
void Settings::saveOptionsView_gui(TreeView &treeView)
{
	GtkTreeIter iter;
	...
	while (valid)
	{
		...
		if (coreSetting >= 0)
		{
			SettingsManager::getInstance()->set((SettingsManager::IntSetting)coreSetting, toggled);
		}
		else
		{
...
void Settings::initAppearance_gui()
{
	{ // Appearance
		...
		addOption_gui(appearanceStore, _("Enable desktop notifications when a download finishes"), "notify-download");
		addOption_gui(appearanceStore, _("Enable desktop notifications when a private message is received"),"notify-pm");
#endif
		/// @todo: Uncomment when implemented
